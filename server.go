...
func (s *Server) handleEvent(sb func(e *pqs.Event), ev *pq.Notification) error {
	if ev == nil {
...
func (s *Server) HandleEvents(ctx context.Context, sb func(e *pqs.Event)) error {
	...
	for {
		...
		case ev := <-events:
			...
			if err := s.handleEvent(sb, ev); err != nil {
				return err
			}
